...
## 🚀 CURRENT PROJECT CONTEXT: CMRE (Custom Mortgage Real Estate)
- **Role**: You are building the "Clean Slate" Next.js app.
- **Design Mandate**: Port from `design/client/src/` to Next.js App Router. **ENHANCE** using "Anti-Safe Harbor" principles.
- **Data Assets**: You have access to **WP CLI**, **WP API**, and **Firecrawl** to pull any missing layout or content details from `custommortgageinc.com`.
- **Pathing Rule**: ALWAYS use **relative paths** (`./`) or `process.cwd()`. NEVER use absolute paths.
- **Key Feature**: The Pricing Engine is pure TypeScript (client-side). Ensure it integrates smoothly with the UI.

### 📁 CMRE FILE REFERENCES (MANDATORY READS)
| File | Purpose |
|------|---------|
| `design/client/src/index.css` | **Design tokens**: `--color-brand-blue: #1daed4`, `--radius: 0rem` |
| `design/client/src/components/*.tsx` | Source components to PORT to Next.js |
| `programs.csv` | 125 loan programs (slug, title, category) |
| `legacy_content.json` | Page content bodies - NO placeholders! |
| `media_inventory.json` | Approved images (logos, flyers) |

### 🎨 CMRE DESIGN TOKENS (COPY EXACTLY)
```css
--color-primary: #1daed4    /* Cyan - MANDATORY */
--color-foreground: #636363  /* Dark Gray */
--radius: 0rem              /* SHARP edges - NO soft corners */
--font-sans: Inter
--font-heading: Bebas Neue
```

### 🚫 CMRE BANNED ELEMENTS
- ❌ Purple/Violet colors (#8b5cf6, #7c3aed)
- ❌ Placeholder text ("Lorem ipsum")
- ❌ Placeholder images ("/placeholder.jpg")
- ❌ border-radius between 4px-8px (the "boring zone")
